  ...
  def receive = {
    ...
    case GimmeWork ⇒ currentEpic match {
      ...
      case Some(epic) ⇒
        ...
        if (epic.iterator.hasNext){
          log.info(s"processing line $currentLine")
          sender ! Work(csvHeader,epic.iterator.next,currentLine)
          currentLine += 1
        } else {
